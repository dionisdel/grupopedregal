      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="bg-[#F5F5F5] border-b border-gray-200">
            ...
            <th className="text-right px-4 py-3 font-semibold text-[#333333]">Precio PVP</th>
          </tr>
        </thead>
        <tbody>
          {products.map((product) => (
            ...
            >
              ...
              <td className="px-4 py-3 text-right font-semibold text-[#E8751A]">
                {formatPrice(product.precio_pvp)}
              </td>
            </tr>
          ))}
        </tbody>
...
export default function ProductExplorerPage() {
  ...
  const [result, setResult] = useState<PaginatedResponse<CatalogProduct> | null>(null);
  ...
  const handleCategorySelect = useCallback(
    (cat: Category | null) => {
      ...
      // Update URL
      if (cat) {
        router.replace(`/productos?category=${cat.slug}`, { scroll: false });
      } else {
  ...
  const handleProductClick = useCallback(
    (slug: string) => {
      router.push(`/productos/detalle?slug=${slug}`);
    },
    [router]
  );
